...
2. 执行composerrequire
composer require es-swoole/common
```
...
3. 安装本包(composer require es-swoole/common)
...
$this->success([])/$this->fail('msg') 用于记录结束请求时的日志
...
*/
//同步发送
Mail::syncSendMail($subject,$body,$to,$isBatch,$connection);
//异步发送
Mail::asyncSendMail($subject,$exceptionBody,$toMail);
//根据进程信息自动选择同步或异步发送
Mail::smartSend($subject,$exceptionBody,$toMail);

4. config包
在需要添加config配置文件时,文件添加在项目根目录下的Config目录中
