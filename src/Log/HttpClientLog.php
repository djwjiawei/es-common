...
class HttpClientLog
{
    ...
    public static function log($param)
    {
        ...
        EsLog::getInstance()->info(
            self::formatHttpLog($param), Logger::HTTP_REQUEST
        );
    }
    ...
    public static function formatHttpLog($param)
    {
        ...
        switch ($logTag) {
            case self::LOG_REQUEST_OUT:
                $return = '[' . $fileName . '-' . $functionName . '-' . $number . ']' . $logTag . '||host=' . $host . '||client_ip=' . $clientIp . '||hintContent=' . $hintContent . '||url=' . $url . '||request=' . $requestParam . '||uid=' . $uid . '||merchant_num=' . $merchantNum . '||response=' . $response . '||elapsed=' . $elapsed . '||code=' . $code . '||msg=' . $msg;
                break;
            ...
            case self::LOG_HTTP_REQUEST:
                $return = '[' . $fileName . '-' . $functionName . '-' . $number . ']' . $logTag . '||host=' . $host . '||client_ip=' . $clientIp . '||hintContent=' . $hintContent . '||url=' . $url . '||request=' . $requestParam . '||uid=' . $uid . '||merchant_num=' . $merchantNum . '||retry=' . $retry . '||msg=' . $msg;
                break;

            default:
                $return = '[' . $fileName . '-' . $functionName . '-' . $number . ']' . $logTag . '||host=' . $host . '||client_ip=' . $clientIp . '||hintContent=' . $hintContent . '||url=' . $url . '||request=' . $requestParam . '||uid=' . $uid . '||merchant_num=' . $merchantNum . '||msg=' . $msg;
        }
